...
def get_slot_status_and_priority(slot_str, current_time):
    ...
    if progress < 0.5:
        print("First half → NORMAL priority")
        return "INSIDE", "0"
    else:
        print("Second half → HIGH priority")
        return "INSIDE", "1"
...
def insert_priority(order_id, status, priority_flag, current_time):
    ...
    check_query = text("SELECT 1 FROM tbl_priority_chatbot WHERE order_id = :order_id")

    insert_query = text("""
        INSERT INTO tbl_priority_chatbot (
    ...
    with live_engine.connect() as conn:
        ...
        conn.commit()
        print("Inserted into tbl_priority_chatbot")
...
def handle_right_branch(order):
    ...
    # ----------------------------
    print("Condition: order_pick_time NULL → checking 3PL")
    ...
    if pl_data is not None:
        ...
        if pl_data["order_picked"]:
            print("3PL.order_picked TRUE → OUT FOR DELIVERY")
